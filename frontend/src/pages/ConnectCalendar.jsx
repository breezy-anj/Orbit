import React, { useContext } from 'react';
import { useNavigate } from 'react-router-dom';
import { Calendar, Shield, Clock, Users } from 'lucide-react';
import { AuthContext } from '../context/AuthContext';

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5001';

export default function ConnectCalendar() {
  const navigate = useNavigate();
  const { user } = useContext(AuthContext);

  const handleConnect = () => {
    
    window.location.href = `${API_URL}/api/auth/google`;
  };

  return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center text-white p-6"
         style={{ fontFamily: "'Inter', 'Outfit', sans-serif" }}>

      {}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[500px] h-[500px]
                        bg-blue-600/8 rounded-full blur-3xl" />
        <div className="absolute bottom-10 right-10 w-72 h-72 bg-purple-600/8 rounded-full blur-3xl" />
      </div>

      <div className="relative max-w-md w-full bg-gray-900/80 backdrop-blur-xl p-10
                      rounded-3xl shadow-2xl text-center space-y-8 border border-gray-800/60">

        {}
        <div className="flex justify-center">
          <div className="w-20 h-20 bg-gradient-to-tr from-blue-500/20 to-purple-500/20
                          rounded-2xl flex items-center justify-center border border-blue-500/20">
            <Calendar size={40} className="text-blue-400" />
          </div>
        </div>

        {}
        <div className="space-y-3">
          <h1 className="text-3xl font-bold tracking-tight">
            {user?.name ? `Hey ${user.name.split(' ')[0]}!` : 'Connect Calendar'}
          </h1>
          <p className="text-gray-400 text-sm leading-relaxed">
            Link your Google Calendar so Flowsphere can find free time, detect hangouts,
            and help you plan meaningful meetups with friends.
          </p>
        </div>

        {}
        <div className="space-y-3 text-left">
          {[
            { icon: Clock,   color: 'blue',   text: 'Read your free/busy windows to find open time' },
            { icon: Users,   color: 'purple', text: 'Detect events with friends as potential meetups' },
            { icon: Shield,  color: 'green',  text: 'Event titles & details are never stored or shared' },
          ].map(({ icon: Icon, color, text }) => (
            <div key={text}
                 className="flex items-center gap-3 bg-gray-800/50 border border-gray-700/50
                            rounded-xl px-4 py-3 text-sm text-gray-300">
              <div className={`w-8 h-8 rounded-lg bg-${color}-500/10 flex items-center justify-center flex-shrink-0`}>
                <Icon size={16} className={`text-${color}-400`} />
              </div>
              {text}
            </div>
          ))}
        </div>

        {}
        <div className="space-y-3 pt-1">
          <button
            id="connect-google-calendar-btn"
            onClick={handleConnect}
            className="w-full flex items-center justify-center gap-3 bg-blue-600 hover:bg-blue-500
                       text-white font-semibold py-3.5 rounded-xl transition-all duration-200
                       shadow-lg shadow-blue-500/25 hover:shadow-xl hover:shadow-blue-500/35
                       hover:-translate-y-0.5 active:translate-y-0"
          >
            <svg viewBox="0 0 24 24" className="w-5 h-5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="white" opacity=".9"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="white" opacity=".9"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="white" opacity=".9"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="white" opacity=".9"/>
            </svg>
            Connect Google Calendar
          </button>

          <button
            id="skip-calendar-btn"
            onClick={() => navigate('/dashboard')}
            className="w-full text-gray-500 hover:text-gray-300 py-2.5 transition-colors
                       font-medium text-sm"
          >
            I'll do this later
          </button>
        </div>
      </div>
    </div>
  );
}
